...
class Spikes(object):
    def __init__(self, board, AI=False):
        ...
        self.AI = AI
        self.board = board
        self.scale = (int(self.board.board_h*0.8/self.N_SPIKES),
                      int(self.board.board_h*0.8/self.N_SPIKES))
        self.gaps = []
    ...
    def random_spikes(self, site):
        ...
        one_empty = random.randint(
            int((self.N_SPIKES-1)*0.3), int((self.N_SPIKES-1)*0.6))
        ...
        for i in range(self.N_SPIKES):
            if i in active_spikes and i != one_empty:
                spikes.append(
                    Spike(site, begin+self.board.top+i*dist, self.scale))
            elif self.AI and i != 0 and i < self.N_SPIKES-4:
